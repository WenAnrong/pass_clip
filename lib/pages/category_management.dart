...
class _CategoryManagementPageState extends State<CategoryManagementPage> {
  ...
  Future<void> _saveEditCategory() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_categories.any((category) => 
          category.name == newCategoryName && category.name != _editingCategory)) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('分类已存在')),
        );
        return;
      ...
      // 更新分类列表
      final categoryIndex = _categories.indexWhere((c) => c.name == _editingCategory);
      if (categoryIndex != -1) {
  ...
  void _showDeleteConfirm(String categoryName) {
    if (categoryName == '未分类') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('默认分类不可删除')),
      );
      return;
    }
                    ...
                    child: Form(
                      ...
                      child: Row(
                        children: [
                          ...
                          ElevatedButton(
                            onPressed: _isEditing ? _saveEditCategory : _addCategory,
                        ...
                        trailing: Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              color: category.name != '未分类' ? Colors.red : Colors.grey,
                            ),
                          ],
                        ),
                      );
